...
export const LEMO_WIZARD_STEPS = [
    'zone',
    'offence_group',
    'offence',
    'offender',
...
export function findFirstMissingFpnField(enviro, options = {}) {
    ...
    if (!isValidUkPostcode(draft.post_code)) {
        return gap('post_code', 'Please provide a valid UK postal code.', 2, 'offender');
    }
...
export function findFirstMissingNotebookField(enviro) {
    const notebook = ensureNotebookEntries(enviro || {});
...
export function enviroStepperStep(enviro, options = {}) {
    return findFirstMissingFpnField(enviro, options)?.stepperStep ?? 8;
}
export function lemoWizardProgress(step) {
...
export function previousLemoWizardStep(step) {
    const index = LEMO_WIZARD_STEPS.indexOf(step);
    if (index <= 0) {
        return null;
    }
...
export function ensureNotebookEntries(enviro) {
    if (!enviro.notebook_entries) {
        enviro.notebook_entries = new NotebookEntry();
    }
    return enviro.notebook_entries;
}
export function hasInProgressFpnDraft(enviro) {
    if (!enviro) {
        return false;
    }
    return !!(enviro.offence_id ||
        enviro.first_name ||
        enviro.last_name ||
...
export function formatDateOfBirth(year, month, day) {
...
function gap(field, message, stepperStep, wizardStep) {
    return { field, message, stepperStep, wizardStep };
}
function hasBwc(enviro) {
...
//# sourceMappingURL=fpn-core-validation.js.map